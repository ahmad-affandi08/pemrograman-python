...
def plan(user_input: str):
    ...
    if teks.startswith("buat quiz dari file "):
        path = teks.replace("buat quiz dari file", "").strip()
        langkah.append(("baca_file", path))
        langkah.append(("ambil_poin", None))
        langkah.append(("buat_soal", None))
        return langkah
...
def act(step, context):
    ...
    if nama_tool == "ambil_poin":
        teks = context.get("last_output", "")
        kalimat = [k.strip() for k in teks.split(".") if k.strip()]
        poin_penting = kalimat[:3] if len(kalimat) >= 3 else kalimat
        return poin_penting

    if nama_tool == "buat_soal":
        poin = context.get("last_output", [])
        if isinstance(poin, list) and len(poin) > 0:
            soal = []
            for i, p in enumerate(poin, 1):
                soal.append(f"Soal {i}: Jelaskan tentang: {p}")
            return "\n".join(soal)
        return "Tidak bisa membuat soal dari data ini."
